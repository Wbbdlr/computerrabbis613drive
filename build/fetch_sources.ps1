...
Write-Host "[1/6] Downloading Otzaria portable app..."
...
Write-Host "[2/6] Downloading Otzaria library (~1.2 GB)..."
$libZip = Join-Path $dl "otzaria_latest.zip"
if (-not (Test-Path $libZip)) { curl.exe -sL --retry 3 -o $libZip $OTZARIA_LIB_URL }

Write-Host "[3/6] Extracting library (~5.5 GB)..."
$lib = Join-Path $usb "Library"
New-Item -ItemType Directory -Force $lib | Out-Null
tar.exe -xf $libZip -C $lib

Write-Host "[4/6] Building browser-reader data (~2.8 GB)..."
python (Join-Path $PSScriptRoot "convert_library.py") --src (Join-Path $lib "אוצריא") --out (Join-Path $usb "Library-Web")

Write-Host "[5/6] Fetching curated English translations from Sefaria (thousands of files -- this is the slowest step)..."
$booksJson = Join-Path $dl "books.json"
if (-not (Test-Path $booksJson)) { curl.exe -sL --retry 3 -o $booksJson "https://raw.githubusercontent.com/Sefaria/Sefaria-Export/master/books.json" }
python (Join-Path $PSScriptRoot "build_translations.py") --books $booksJson --out (Join-Path $usb "Library-Web")

Write-Host "[6/6] Fetching the English-translated Siddur Sefard..."
python (Join-Path $PSScriptRoot "fetch_siddur.py") --out (Join-Path $usb "Library-Web")

Write-Host "Done. The complete drive folder is: $usb"
Write-Host "Two more one-time steps are documented in README.md (dictionaries, full-text search index)."
